...
namespace FraudEngine.Tests.Rules
{
    public class RapidTransactionsRuleTests
    {
        ...
        public async Task EvaluateAsync_FewerThanThreshold_ProducesNoAlert()
        {
            ...
            var rule = new RapidTransactionsRule(repo, countThreshold: 5, window: TimeSpan.FromMinutes(1));
        ...
        {
            var repo = new InMemoryRepository();
            var rule = new RapidTransactionsRule(repo, countThreshold: 3, window: TimeSpan.FromSeconds(30));
